...
public class World {
    ...
    static boolean[][][] hollow(int width, int height, int depth, int straightness, Random r) {
        ...
        int n = ( width * height * depth ) / ( 3 );
        int x = 0, y = 0, z = 0, straight = 0, direction = 0;
        for(int i = 0; i < n; i++) {
            hollow[z][y][x] = true;
            if(--straight < 0) {
                straight = straightness + ( r.nextInt(2) * ( straightness / 2 ) );
        ...
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < n; j++) {
                for(int k = 0; k < n; k++) {
                    ...
                    mesh.scaleTextureCoordinates(new Vector2f(units / 4, units / 4));
                    Geometry geom = new Geometry("Chunk " + k + "." + j + "." + i, mesh);
                    geom.setMaterial(mat);
                    geom.setLocalTranslation(i * ( width / n ) * units,
                                             j * ( height / n ) * units,
                                             k * ( depth / n ) * units);
            ...
            for(int j = heightOffset; j < heightOffset + height; j++) {
                for(int k = widthOffset; k < widthOffset + width; k++) {
                    ...
                    if(start != null && ( !rle || current || start.y != pos.y )) {
                        ...
                        Vector3f half = new Vector3f(.5f * ( size.x + units ),
                                                     .5f * ( size.y + units ),
                                                     .5f * ( size.z + units ));
